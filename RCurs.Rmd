...
Podemos informarnos acerca de la longitud, clase, estructura y atributos de un objeto por medio de las funciones `length`, `class`, `str` y `attributes` respectivamente. La función `names` nos permite conocer los nombres de los elementos de un objeto, como pueden ser las variables de un data frame, la función `levels` evaluada en un factor retorna las categorías del factor y la función `dim` retorna las dimensiones de un objeto, como pueden ser las filas y columnas de una matriz o de un data frame.
...
## Transformación de variables en una base de datos con la librería `dplyr`
