...
## Correlación y khi (o ji o chi)-cuadrado y correlación
...
## Prueba Wilcoxon-Mann-Whitney (o test U de Mann-Whitney)
...
```{r, echo=TRUE, eval=TRUE, fig.height=2.45, fig.width=4}

p <- dataw1 %>% filter(!is.na(hea1)) %>% 
  ggplot(aes(x = hea1)) + 
  geom_histogram(binwidth = 15, color = "blue", fill = "yellow")
plot(p)
```
...
dataw1 %>% filter(!is.na(hea1)) %>% 
  ggplot(aes(x = edu1, y = hea1)) + 
  geom_boxplot() +
  coord_flip()
...
- *En el histograma: Qué ocurre si cambiamos el valor de `binwidth`? Y si no pasamos ningún valor? Y si usamos el argumento `bins` en lugar de `binwidth`?*
- *En el boxplot: Qué ocurre si prescindimos de `coord_flip`?*
- *Ejecutar `ggsave(filename = "hi_grafica.png", device = "png, width = 15, height = 20, units = "cm", dpi = 300)`. Qué podemos ver en el fichero hi_grafica.png?*
- *Ejecutar `ggsave(filename = "hi_grafica2.png", plot = p, device = "png, width = 15, height = 20, units = "cm", dpi = 300)`. Qué podemos ver en el fichero hi_grafica2.png?*
