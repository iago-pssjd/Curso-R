...
- Por qué R?
    ...
    + Porque cuando se producen errores, en general los mensajes son bastante informativos (a diferencia de los crípticos errores en otros lenguajes, v. gr. Python)
...
- Para empezar, R permite realizar las operaciones de cualquier calculadora científica.
...
- Habitualmente es suficiente ejecutar el nombre de un objeto para ver el objeto. En ciertas ocasiones conviene evaluarlo explícitamente con la función `print`.
...
- *Buscar en la ayuda de las funciones presentadas (por ejemplo `paste`, `seq` o `rep`) qué opciones presentan según como se especifiquen sus argumentos. Para qué sirven las funciones `seq_len` y `seq_along`?*
...
- *Buscar todas las variables categóricas en las 3 bases de datos y transformar estas bases de datos de manera que esas variables sean factores. Mostrar las frecuencias de las categorías de algunas de esas variables. Transformar la variable `q0002_hhid` a cadena de carácteres.*
- *Qué hace la función transmute?*
- *Qué ocurre si escribimos un `-` delante de `end_with` en `dataw1 %>% select(ends_with("1")) %>% names()`?*
- *De qué otras funciones similares a `ends_with` disponemos?*
- *Seleccionar todas las variables numéricas sin nombrarlas*
- *Comprender los distintos pasos en la instrucción `dataw1 %>% distinct(dep1, score_lon1) %>% group_by(dep1) %>% summarise(n = n())` y las distintas funciones mencionadas referentes a estadísticos. Mirar también qué hace y cómo se puede usar la función `range`*
...
Cuando se combinan las filas mediante `bind_rows`, las columnas que sólo están en una base de datos se llenan con missings. Por ejemplo, es el caso de las variables sociodemográficas, que en el dataframe `acv` sólo tienen datos en las filas correspondientes a la ola 1, que son las filas obtenidas de `ac1`. 
...
- *Qué variables de acv no tienen necesariamente los mismos valores en las distintas olas?*
...
Podéis consultarme vuestras dudas a través de mi dirección de email: iago.gine at sjd.es